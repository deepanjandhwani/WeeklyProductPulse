"""
tests/test_cache_manager.py — Unit tests for src/cache_manager.py
...
def test_window_returns_10_weeks():
...
def test_window_starts_9_weeks_before():
    window = compute_10_week_window("2026-W11")
    assert window[0] == "2026-W02"
...
def test_expire_removes_weeks_outside_window():
    save_week_cache("2026-W01", [make_review(iso_week="2026-W01")])  # old week
    save_week_cache("2026-W11", [make_review(iso_week="2026-W11")])  # in window
    window = compute_10_week_window("2026-W11")
    expired = expire_old_weeks(window)
    assert "2026-W01" in expired
    assert load_week_cache("2026-W01") is None

"""
tests/phase1/test_filter.py — Unit tests for phase1_ingestion/filter.py

Run with:  pytest tests/phase1/test_filter.py -v
"""
...
from phase1_ingestion.filter import filter_reviews, REVIEW_COLUMNS, _word_count, _iso_week_label
...
def test_skips_review_older_than_lookback_window():
    days_older_than_lookback = (config.LOOKBACK_WEEKS * 7) + 1
    raw = [make_raw(days_ago=days_older_than_lookback)]
    result, stats = filter_reviews(raw)
    assert len(result) == 0

"""
src/cache_manager.py — Weekly CSV cache for Play Store reviews.
...
* Expire weeks that are no longer in the 10-week rolling window.
...
---------------------------------
   ...
   ...
   Week N-9 →  load from cache (do not re-fetch)
   Week N-10 → expired and deleted

Usage (standalone)
------------------
    python -m src.cache_manager --list
    python -m src.cache_manager --expire
"""
...
import config
from src.filter import REVIEW_COLUMNS
...
def compute_10_week_window(current_week: str) -> list[str]:
    """
    Return a list of the 10 ISO-week strings ending at current_week (inclusive),
...
def expire_old_weeks(current_window: list[str]) -> list[str]:
    """
    Delete cache files for weeks that are no longer in the 10-week window.

    Args:
        current_window: The list of 10 week-strings that should be kept.
    ...
    for week in all_cached:
        if week not in current_window:
            ...
            expired.append(week)
            logger.info(
                f"Expired cache for {week} (outside 10-week window).",
                extra={
                    "phase": "cache_manager",
...
def save_consolidated(df: pd.DataFrame, iso_week: str) -> Path:
    """
    Save the merged 10-week DataFrame to data/consolidated/<iso_week>_full.csv.
...
if __name__ == "__main__":
    ...
    from src.scraper import get_current_iso_week
    ...
    parser.add_argument("--expire", action="store_true", help="Expire weeks outside the 10-week window")
    ...
    if args.expire:
        window = compute_10_week_window(args.week)
        print(f"\nCurrent 10-week window: {window}")

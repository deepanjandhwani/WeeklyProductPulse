"""
src/scraper.py — Play Store review fetcher for IndMoney (in.indwealth).

Responsibilities
----------------
* Fetch reviews for a given ISO-week date range using google-play-scraper.
* Sort by MOST_RELEVANT so the highest-signal reviews surface first.
* Return raw review dicts — filtering is the responsibility of filter.py.

Usage (standalone)
------------------
    python -m src.scraper --week 2026-W11
    python -m src.scraper --week 2026-W11 --log-level DEBUG
"""
...
def fetch_reviews_for_week(iso_week: str) -> list[dict]:
    ...
    Strategy
    --------
    * Scraper sorts by MOST_RELEVANT, so the most-upvoted / most-useful reviews
      come first — consistent with the architecture constraint.
...
if __name__ == "__main__":
    ...
    if raw:
        sample = raw[0]
        print(f"Sample: ★{sample.get('score')} | 👍{sample.get('thumbsUpCount')} | {sample.get('content', '')[:80]}")

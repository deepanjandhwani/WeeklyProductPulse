...
| Concern               | Mitigation                                                                                    |
| --------------------- | --------------------------------------------------------------------------------------------- |
| **API keys exposure** | Stored in `.env` (git-ignored), **Render environment variables** for the web app, and **GitHub Secrets** for CI; each environment reads its own source |
...
| **Google OAuth (MCP)** | **Google Docs:** `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET`; token in `~/.config/google-docs-mcp/token.json` locally or **`GOOGLE_DOCS_MCP_TOKEN_JSON`** on Render / in CI. **Gmail MCP email:** same OAuth client as used for auth; **`GMAIL_MCP_CREDENTIALS_JSON`** on **Render** (full JSON including **refresh_token**) is required for `EMAIL_TRANSPORT=mcp` in production — without it, Google clients report *No access, refresh token, API key or refresh handler callback is set*. `entrypoint.sh` writes `gcp-oauth.keys.json` and `credentials.json` under `/root/.gmail-mcp/` when the corresponding env vars are set. |


---

*Document updated 2026-03-28 — v2.2: Document **`GMAIL_MCP_CREDENTIALS_JSON`** on **Render** as required for production MCP email; clarify **`entrypoint.sh`** paths; replace stale “Railway” references with **Render**. v2.1: Vercel **Root Directory `frontend`**; **Lora + DM Sans**; brand asset in **`frontend/public/assets/images/`**. v2.0: backend on Render; **`requirements-app.txt`**. v1.9 Next.js dashboard.*